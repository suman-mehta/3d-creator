<div align="center">

  <a href="https://sumanmehta.in/">
    <img src="https://raw.githubusercontent.com/suman-mehta/Bihar-Bijli/refs/heads/main/suman%20mehta%203D.png" width="150" height="150" alt="Suman Mehta — 3D Creator & Motion Designer" style="border-radius: 50%;" />
  </a>

  # Suman Mehta — 3D Creator & Creative Technologist

  <p align="center">
    <strong>Crafting Spatial Web Experiences, High-Fidelity 3D Systems & Motion Design</strong>
  </p>

  <p align="center">
    <a href="https://sumanmehta.in/"><img src="https://img.shields.io/badge/🌐_Official_Portfolio-sumanmehta.in-00D4FF?style=for-the-badge&labelColor=0a0a0a" alt="Official Website sumanmehta.in" /></a>
    <a href="https://www.linkedin.com/in/suman-mehta-in/"><img src="https://img.shields.io/badge/LinkedIn-suman--mehta--in-0077B5?style=for-the-badge&logo=linkedin&logoColor=white&labelColor=0a0a0a" alt="LinkedIn Profile" /></a>
    <a href="https://x.com/itz_suman_mehta"><img src="https://img.shields.io/badge/Twitter/X-@itz__suman__mehta-black?style=for-the-badge&logo=x&logoColor=white&labelColor=0a0a0a" alt="Twitter Profile" /></a>
    <a href="https://www.instagram.com/suman_mehta.me"><img src="https://img.shields.io/badge/Instagram-@suman__mehta.me-E4405F?style=for-the-badge&logo=instagram&logoColor=white&labelColor=0a0a0a" alt="Instagram Profile" /></a>
    <a href="https://wa.me/917739105800"><img src="https://img.shields.io/badge/WhatsApp-Connect-25D366?style=for-the-badge&logo=whatsapp&logoColor=white&labelColor=0a0a0a" alt="WhatsApp Contact" /></a>
  </p>

</div>

---

## 🌟 About the Entity

**[Suman Mehta](https://sumanmehta.in/)** (also known as **Suman Kumar**) is an Indian **3D Creator, Motion Designer, and Creative Technologist** based in Katihar, Bihar. 

He specializes in custom-coded spatial 3D web environments, photorealistic rendering, physics-driven animations, and signature visual brand systems. His work bridges computational geometry and modern web technology to replace standard static layouts with interactive visual storytelling.

* **Primary Canonical Webpage**: [https://sumanmehta.in/](https://sumanmehta.in/)
* **Entity Type**: [`Person`](https://schema.org/Person) (`@id: https://sumanmehta.in/#person`)
* **Role**: 3D Creator, Motion Designer & Creative Technologist
* **Location**: Katihar, Bihar, India 🇮🇳

---

## 🚀 Featured Projects

| Project | Category | Focus & Visual Architecture | Experience |
| :--- | :--- | :--- | :--- |
| **Nextlevel Studio** | Client Showcase | Interactive spatial 3D showcase with reactive scroll physics and dynamic lighting. | [View Live](https://sumanmehta.in/#projects) |
| **Aura Brand Identity** | Personal Project | Signature geometric brand system, procedural material shaders, and lighting setups. | [View Live](https://sumanmehta.in/#projects) |
| **Solaris Digital** | Client Showcase | Photorealistic 3D rendering and conversion-focused digital interface. | [View Live](https://sumanmehta.in/#projects) |

---

## 🛠️ Technology Stack & Creative Systems

* **Spatial Web & Physics**: Custom 3D Web Frameworks, WebGL, WebGPU, Reactive Geometry.
* **Frontend Engineering**: React 19, TypeScript, Vite, Tailwind CSS, Framer Motion.
* **Creative Tools**: Blender, Cinema 4D, After Effects, Figma.
* **Search & Entity Architecture**: Schema.org JSON-LD Linked Data Graph, OpenGraph Profile tags, Google Image Sitemaps.

---

## 🔍 Semantic Search & Knowledge Graph Attributes

This repository serves as the official source code for **[sumanmehta.in](https://sumanmehta.in/)**, engineered to maintain authoritative entity signals:

```json
{
  "@context": "https://schema.org",
  "@type": "Person",
  "@id": "https://sumanmehta.in/#person",
  "name": "Suman Mehta",
  "alternateName": ["Suman Kumar", "Suman", "itz_suman_mehta", "suman-mehta"],
  "url": "https://sumanmehta.in/",
  "jobTitle": "3D Creator, Motion Designer & Creative Technologist",
  "sameAs": [
    "https://www.linkedin.com/in/suman-mehta-in/",
    "https://github.com/suman-mehta",
    "https://suman-mehta.github.io/",
    "https://suman-mehta.github.io/My-website-/",
    "https://www.instagram.com/suman_mehta.me",
    "https://x.com/itz_suman_mehta",
    "https://wa.me/917739105800",
    "https://sumanmehta.in/"
  ],
  "knowsAbout": [
    "3D Computer Graphics",
    "Motion Graphics",
    "3D Modeling",
    "WebGL",
    "WebGPU",
    "Front-End Web Development",
    "Brand Architecture"
  ]
}
```

---

## 📬 Connect & Collaborate

* **Website**: [https://sumanmehta.in/](https://sumanmehta.in/)
* **LinkedIn**: [suman-mehta-in](https://www.linkedin.com/in/suman-mehta-in/)
* **Email**: [sumankumardphs438@gmail.com](mailto:sumankumardphs438@gmail.com)
* **X / Twitter**: [@itz_suman_mehta](https://x.com/itz_suman_mehta)
* **Instagram**: [@suman_mehta.me](https://www.instagram.com/suman_mehta.me)
* **GitHub**: [@suman-mehta](https://github.com/suman-mehta)
* **WhatsApp**: [+91 7739105800](https://wa.me/917739105800)

---

<div align="center">
  <p>© 2026 <strong><a href="https://sumanmehta.in/">Suman Mehta</a></strong>. All Rights Reserved.</p>
</div>
